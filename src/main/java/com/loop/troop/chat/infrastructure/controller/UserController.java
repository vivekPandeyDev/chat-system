...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<UserResponseDto>> createUser(@RequestBody CreateUserRequestDto request) {
        var user = userUserCase.registerUser(new CreateUserCommand(request.getUsername(), request.getEmail()));
        var userFetchResponse = new ApiResponse<>(true, "User registered successfully", UserMapper.toResponseDto(user, userUserCase.fetchProfileUrl(user)));
    ...
    public ResponseEntity<ApiResponse<UserResponseDto>> getUser(@PathVariable String userId) {
        ...
        var userFetchResponse = new ApiResponse<>(true, "User fetched successfully", UserMapper.toResponseDto(user, userUserCase.fetchProfileUrl(user)));
        return ResponseEntity.ok(userFetchResponse);
    }


    @GetMapping
    public ResponseEntity<PageResponse<UserResponseDto>> getAllUsers(@RequestParam(defaultValue = "0") Integer offset, @RequestParam(defaultValue = "10") Integer size, @RequestParam(defaultValue = "name") String sortBy, @RequestParam(defaultValue = "ASC") String sortDir) {
        var query = new PaginationQuery(offset, size, sortBy, sortDir);
        var pageResponse = userUserCase.fetchUsers(query);
        var userResponseDtoList = pageResponse.content().stream().map(user -> UserMapper.toResponseDto(user, null)).toList();
    ...
    public ResponseEntity<ApiResponse<UserResponseDto>> updateStatus(@PathVariable String userId, @PathVariable String status) {
        ...
        var user = userUserCase.fetchUserByUserId(userId).map(domain -> UserMapper.toResponseDto(domain, userUserCase.fetchProfileUrl(domain))).orElseThrow(() -> UserServiceException.userNotFound(userId));
        var response = new ApiResponse<>(true, "User status updated successfully", user);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/upload/profile/{userId}")
    public ResponseEntity<ApiResponse<UserResponseDto>> uploadProfileImage(@ValidUUID(message = "User ID must be a valid id") @PathVariable(value = "userId", required = false) String userId, @NotNull(message = "File be provided") @RequestParam(value = "file", required = false) MultipartFile file) throws IOException {
        if (file.isEmpty()) {
        ...
        var fileUploadCommand = new FileUploadCommand(filePath, file.getInputStream(), file.getSize(), file.getContentType());
        var profileUrl = userUserCase.uploadUserProfile(userId, fileUploadCommand);
        var user = userUserCase.fetchUserByUserId(userId).map(domain -> UserMapper.toResponseDto(domain, profileUrl)).orElseThrow(() -> UserServiceException.userNotFound(userId));
